...
public class SoUtil {
	...
	public static boolean isUnix() {
		return File.listRoots().length == 1 && File.listRoots()[0].getAbsolutePath().equals("/") && new File("/usr").exists();
	}
	...
	public static boolean is64Bit() {
		...
		try {
			...
			return JOptionPane.showConfirmDialog(null, "Is your machine is 64-Bit ?", "Is 64-Bit", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION;
		}
	}
	...
	public static KeyStroke getKeyStrokeForAll(String key) {
		...
		if (name.equals("f1")) {
			...
			key = key.substring(0, key.length() - 1) + Character.toUpperCase(key.charAt(key.length() - 1));
		}
