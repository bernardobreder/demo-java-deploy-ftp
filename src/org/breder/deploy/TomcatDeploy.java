...
public abstract class TomcatDeploy extends Deploy {
	...
	public void init() throws Exception {
		...
		this.buildWar(new File(outputDir, "server.war"), this.getTomcatProject(), this.getServerDirs());
		this.publish(String.format("webapps/%s.war", this.getProjectName()), new File(super.outDir, "server.war"));
		this.remove(outputDir);
	}

	protected void buildWar(File fileout, File tomcat, File... others) throws IOException {
		...
		try {
			...
			for (File file : tomcat.listFiles()) {
				...
				if (!(name.startsWith("."))) {
					if (file.isDirectory()) {
						if (!(name.equals("out") || name.equals("tmp") || name.equals("WEB-INF") || name.equals("work"))) {
